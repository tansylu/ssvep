...
highest_pattern = 'models_vs_highest_*'  # Pattern for highest similarity pruning
...
def extract_data_from_directories(pattern):
    ...
    for result_dir in result_dirs:
        ...
        try:
            ...
            percentage_str = parts[-1]  # Get the last part which should be the percentage
            
            # Convert to correct percentage value
            # Handle special cases for two-digit values starting with 0
            if len(percentage_str) == 2 and percentage_str.startswith('0'):
                # "01" means 0.1%, "02" means 0.2%, etc.
                percentage = float(percentage_str) / 10
            else:
                # "1" means 1%, "10" means 10%, etc.
                percentage = int(percentage_str)
            
            print(f"Processing directory: {dir_name}, extracted percentage: {percentage}")
            
            # Look for summary CSV files
            summary_files = glob.glob(os.path.join(result_dir, '*_summary.csv'))
            if summary_files:
                print(f"  Found summary file: {os.path.basename(summary_files[0])}")
                ...
                if 'Top1_Match_Percentage' in df.columns:
                    ...
                    print(f"  Extracted match percentage: {match_pct}")
                else:
                    print(f"  Warning: 'Top1_Match_Percentage' column not found in {summary_files[0]}")
                    print(f"  Available columns: {df.columns.tolist()}")
            else:
                print(f"  No summary files found in {result_dir}")
...
print("\nExtracting data for similarity-based pruning (lowest)...")
similarity_percentages, similarity_matches = extract_data_from_directories(similarity_pattern)

print("\nExtracting data for random pruning...")
random_percentages, random_matches = extract_data_from_directories(random_pattern)

print("\nExtracting data for similarity-based pruning (highest)...")
highest_percentages, highest_matches = extract_data_from_directories(highest_pattern)

print(f"\nSummary:")
print(f"Found {len(similarity_percentages)} similarity pruning results: {similarity_percentages}")
print(f"Found {len(random_percentages)} random pruning results: {random_percentages}")
print(f"Found {len(highest_percentages)} highest similarity pruning results: {highest_percentages}")
...
plt.ylabel('Top-1 Match Percentage (%)', fontsize=12)
plt.title('Comparison of Pruning Methods: Top-1 Prediction Match with Original Model', fontsize=14)
...
# Create DataFrame
comparison_data = pd.DataFrame(combined_data, columns=['Pruning_Percentage', 'Top1_Match_Percentage', 'Pruning_Method'])
